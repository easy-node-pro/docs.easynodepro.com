...
Update your firewall to change port 22 to the new port # you chose at this point. Here's a reminder on our [Harmony](/harmony/companion#firewall-setup-notes) firewall notes. Sub your new port for 22 in the guides.
...
<Tabs
  items={["Run Harmony Toolbox", "Show Harmony Stats"]}
  defaultIndex="0"
>
  <Tab>
...
Docker is required to run some validators and other software. With your new server online and logged in to the user you'll be running the programs as, install docker on your Ubuntu 20.04/22.04LTS system with the following command. If you user isn't `serviceuser` update that to be your username in the following code before running it on your server:

```text copy
sudo apt update -y && sudo apt install apt-transport-https ca-certificates curl software-properties-common -y && curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo apt-key add - && sudo add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/ubuntu focal stable" && sudo apt install docker-ce -y && sudo usermod -aG docker serviceuser
```
